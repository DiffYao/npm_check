...
function nodeToString(node, label) {
	...
	// return `${node.type}${suffix} (${generate(node).code})`;
...
module.exports = {
				...
				for (let i = 0; i < state.currentSegments.length; ++i) {
					...
					if (leaving) {
						...
						if (
							...
							str = nodeToString(node, void 0);
							segInternal.nodes[last] = str;
							segInternal.nodeMap[last] = {
								str: str,
								node: node,
							};
						} else {
							str = nodeToString(node, "exit");
							segInternal.nodes.push(str);
							segInternal.nodeMap.push({
								str: str,
								node: node,
							});
						}
					} else {
						str = nodeToString(node, "enter");
						segInternal.nodes.push(str);
						segInternal.nodeMap.push({
							str: str,
							node: node,
	 ...
	 */
	dumpDot: /* istanbul ignore next */ function (codePath) {
		let text =
			"\n" +
			"digraph {\n" +
